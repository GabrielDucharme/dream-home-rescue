...
export const QuiNousSommesClient: React.FC<QuiNousSommesClientProps> = ({ adoptionLimit = 4 }) => {
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 z-10 text-white">
            ...
            >
              <div className="flex justify-between items-end mb-2">
                <div>
                  <h3 className="text-3xl font-bold mb-1 text-white group-hover:text-amber-200 transition-colors">
                    {dog.name}
                  </h3>
                ...
                {(dog.adoptionDate || dog.successStory?.adoptionDate) && (
                  ...
                  >
                    <div className="bg-gradient-to-r from-[#26483B] to-[#1c3529] text-white text-sm px-4 py-2 rounded-lg border border-white/10 shadow-md">
                      <div className="text-xs uppercase font-medium text-amber-200 mb-1 tracking-wide">
                        Adopté le
                      </div>
              ...
              {(dog.adoptionFamily || dog.successStory?.family) && (
                <div className="flex items-center mt-2 bg-[#26483B]/20 px-3 py-2 rounded-lg backdrop-blur-sm">
                  <svg
                    ...
                    className="h-4 w-4 mr-2 text-[#26483B]/90"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
          ...
          {adoptedDogs.map((_, index) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                index === currentDog ? 'bg-amber-400 scale-125' : 'bg-white/50 hover:bg-white/70'
              }`}
